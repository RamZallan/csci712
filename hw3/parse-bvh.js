/*
    Edited version of https://github.com/mikolalysenko/parse-bvh/
*/
...
function parseBVH(bvhFile) {
    ...
    function parseFrame() {
        var tokens = getTokens();
        assert(
            tokens.length === parameterCount,
            `invalid number of channels for frame ${frames.length}`
        );
    ...
    function parseMotion() {
        ...
        assert(
            frameTimeTokens.length === 2 && frameTimeTokens[0] === "Time:",
            "invalid frame time"
        );
